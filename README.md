...
第二，在产品逻辑看，区块链产品的处理对象源自系统本身，这导致区块链产品是封闭的虚拟世界。

目前所有区块链项目的根本缺陷都是假设上链数据的可信。但实际上区块链系统最大的不确定性来自人。人们为了利益在数据获取源头甚至计算执行过程中篡改数据是无法监督的。只有保证上链数据的可信性，才能从根本上解决区块链项目的可信性问题。
