...
人们希望大规模应用比特币背后的技术，来解决社会各式各样的委托代理人问题。我们认为阻碍比特币技术大规模应用的关键原因有两个：

第一，在技术逻辑看，区块链技术的单一全局共识机制本身约束了应用场景的范围。一方面，全局信任的机制限制了应用场景的多样性。另一方面，单一的统一共识算法决定了极少数出块节点成了系统容量的瓶颈。

第二，在产品逻辑看，区块链产品的处理对象源自系统本身，这导致区块链产品是封闭的虚拟世界。目前大部分区块链项目的根本缺陷恰恰是违背了区块链的这个前提假设，处理的数据来自系统之外，从而产生所谓“上链数据”的可信性问题。人们为了利益在数据获取源头甚至计算执行过程中篡改数据是无法监督的，而这是人的本性，不能靠自觉。边缘审计解决数据可信问题也是本末倒置，成本太高。

# 思路

我们的思路是：代理人角色有两个作用，第一是共识的代言和执行。第二是分工带来的效率。比特币给我们的最大资产是利用数学算法这个无利益第三方来约束利益实体的权利，减少不确定性。对于代理人，我们只需要用数学算法来代表规则约束实体。这样既能保证分工的效率，同时保证规则执行结果的可预期性。在对等网络中，每个实体既是参与者，又是代理人。所以，我们对代理人的约束实际上是对所有对等网络节点的约束。

为了适应业务场景的无限复杂性，我们希望数学算法形成的约束空间维度越高越好。或者说，任何单一的技术都有局限性。我们需要一系列的技术集合来满足场景的复杂性和确保预期。

当代计算机都遵循冯·诺伊曼结构，将整个系统分为输入输出（IO）、计算（运算器与控制器）、存储三个部分。多数区块链技术通过点对点分布式存储、非对称加密以及链式块存储满足存储可信，但IO与计算都只限定了特定类型来解决可信问题。单纯的算法无法解决任意数据类型的IO以及计算也是多数区块链存在的问题。要实现最大范围的信任根，我们需要解决IO、计算以及存储的可信问题。同时，实体（人、组织、甚至AI等）作为最重要的IO源之一，同样需要纳入信任范围。
...
如果只是权利声明而没有实质性的保护，那么和现有互联网对于权利保护只能走法律途径没有不同，无法把维权成本大幅降低。要做到实质性的保护，我们需要将硬件级的信任根植入到系统当中，实现计算过程可信。信任空间当中的可信执行环境应当满足以下条件：
